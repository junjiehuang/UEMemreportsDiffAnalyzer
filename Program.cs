    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (args == null || args.Length == 0)
            {
                ...
                {
                    "-1f",
                    string.Format("{0}/MemReports/01-17.19.54.memreport", CurPath),
                    "-2f",
                    string.Format("{0}/MemReports/30-18.40.46.memreport", CurPath),
                    "-o",
            ...
            if (string.IsNullOrEmpty(diffCSVPath) == false)
            {
                if(FileNameLst.Count == 2)
                    diffCSVPath += string.Format("/Diff_{0}-{1}.csv", FileNameLst[1], FileNameLst[0]);
                else if(FileNameLst.Count == 1)
                    diffCSVPath += string.Format("/Diff_{0}.csv", FileNameLst[0]);
                Console.WriteLine( string.Format("Compare Result File [{0}]: ", diffCSVPath) );
            }
            else
            {
                if(FileNameLst.Count == 2)
                    diffCSVPath = System.IO.Directory.GetCurrentDirectory() + string.Format("/Diff_{0}-{1}.csv", FileNameLst[1], FileNameLst[0]);
                else if(FileNameLst.Count == 1)
                    diffCSVPath = System.IO.Directory.GetCurrentDirectory() + string.Format("/Diff_{0}.csv", FileNameLst[0]);
                Console.WriteLine( string.Format("Compare Result File [{0}]: ", diffCSVPath) );
            }
